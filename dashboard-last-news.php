...
<?php
				...
				foreach ($feed->get_items() as $item)
				{
					...
					$date2  = mysql2date($date_format,$item->get_date('Y-m-d H:i:00')); 	
					$fiid   = $item->get_feed(); 	
					$from   = $fiid->get_title();
?>
<li  style='line-height:100%;margin:0;<?php echo $style ?>'><?php if ($img) echo "<table style='margin:0;padding:0;'><tr style='margin:0;padding:0;'><td style='margin:0;padding:0;'>\n" . $img; ?><span class='lastnews'><a class='lastnews' href='<?php echo $item->get_permalink(); ?>' title='<?php echo $from; ?>' target='_blank'><?php echo $titl1; ?></a> &#8212; <abbr title="<?php echo $date2; ?>"><?php echo $date; ?></abbr></span><?php if ($img) echo "</tr></td></table>"; ?></li>
<?php
					$z++;
					if ( $z > $maxlines )  break;
				}
...
<?php
	...
	function get_image($item)
	{
		...
		if ($img == array())
		{
// try to filter any icons of social bookmarkers 
			$needles = array ('bookmark.gif');
			...
			if (isset($match2s [0] [1])) 							$wimg = str_replace(' ', '%20',$match2s [0] [1]);
...
/***************************************/
	function in_string($haystack,$needles)
	{
		foreach ($needles as $needle) if ((stripos($haystack,$needle) !== false)) return true;
		return false;
	}
...
/***************************************/
	...
	{
		$w = array();
// default wp2.5 we are always in !!!
		$dashboard_last_news_widget_count = (get_option('dashboard-last-news-widget-count')) ? get_option('dashboard-last-news-widget-count') : 1;
